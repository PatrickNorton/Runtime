...
namespace Constants {
    StdType::StdType(FileInfo* file, uint16_t, std::map<Operator, uint32_t> operators, std::map<Operator, uint32_t> staticOps,
            std::map<std::string, uint32_t> methods, std::map<std::string, uint32_t> staticM)
            : file(file), operators(std::move(operators)), staticOperators(std::move(staticOps)) {
    ...
    Variable StdType::getMethod(const std::string& name, StdVariablePtr self) {
        if (methods.count(name)) {
            return std::make_shared<Method<StdVariable>>(self, file, methods.at(name));
        } else {
    ...
    Variable StdType::getMethod(Operator op, StdVariablePtr self) {
        return std::make_shared<Method<StdVariable>>(self, file, operators.at(op));
    }

    Variable StdType::operator[](std::pair<std::string, Runtime*> pair) {
        return std::make_shared<StdFunction>(file, staticMethods.at(pair.first));
    }
    ...
    Variable StdType::operator[](std::pair<Operator, Runtime*> pair) {
        ...
        return std::make_shared<StdFunction>(file, staticOperators.at(pair.first));
    }
