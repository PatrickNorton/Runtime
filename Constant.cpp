...
Variable Constants::IntConstant::callOperator(Operator o, std::vector<Variable> args) {
    switch (o) {
        ...
        case Operator::ADD: {
            ...
            return std::make_shared<_Variable>(IntConstant(result));
        }
        ...
        case Operator::U_SUBTRACT: {
            assert(args.empty());
            return std::make_shared<_Variable>(IntConstant(-value));
        }
        ...
        case Operator::EQUALS:
            for (const auto& arg : args) {
                if (arg->toInt() != value) {
                    return std::make_shared<_Variable>(BoolConstant(false));
                }
            }
            return std::make_shared<_Variable>(BoolConstant(true));
        ...
        case Operator::LEFT_BITSHIFT: {
            assert(args.size() == 1);
            auto arg = args[0];
            static auto maxBsSize = std::make_shared<_Variable>(Bigint(std::numeric_limits<size_t>::max()));
            if ((*arg).callOperator(Operator::GREATER_THAN, {maxBsSize})) {
                throw std::runtime_error("Bitshift value greater than max allowed");
            }
            return std::make_shared<_Variable>(value << static_cast<size_t>(arg->toInt()));
        }
